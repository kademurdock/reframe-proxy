...
test('production HTTP keeps ordinary contrast banter and observes it without a repair',async()=>{
 const dir=fs.mkdtempSync(path.join(os.tmpdir(),'rewrite-wire-')),receipt=path.join(dir,'requests.jsonl');fs.writeFileSync(receipt,'');
 const child=spawn(process.execPath,['--require',path.join(__dirname,'test-fixtures/model.cjs'),'server.js'],{cwd:__dirname,env:{...process.env,PORT:'31869',OPENROUTER_KEY:'offline',PROXY_SHARED_SECRET:'offline',ZAI_KEY:'',MODEL_WIRE_RECEIPT:receipt,MODEL_WIRE_ORIGINAL:"%%%amused%%% That's not modest. That's enormous."},stdio:['ignore','pipe','pipe']});
 let log='';child.stdout.on('data',x=>log+=x);child.stderr.on('data',x=>log+=x);
 try{
  let ready=false;for(let i=0;i<80;i++){try{ready=(await fetch('http://127.0.0.1:31869/health')).ok;}catch{}if(ready)break;await new Promise(r=>setTimeout(r,50));}assert.ok(ready,log);
  const r=await fetch('http://127.0.0.1:31869/chat/completions',{method:'POST',headers:{'Content-Type':'application/json',Authorization:'Bearer offline'},body:JSON.stringify({model:'x-ai/grok-4.20',stream:false,max_tokens:300,messages:[{role:'system',content:'Speak casually with your friend.'},{role:'user',content:'What do you think? [INSTANT]'}]})});
  assert.equal(r.status,200);const out=(await r.json()).choices[0].message.content;
